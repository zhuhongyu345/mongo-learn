...
/**
 * A simple class to centralize logic needed to locate config files on the
 * system.
 *
 * @author Steve
 */
public final class ConfigHelper {
     ...
     * makes a call to {@link #locateConfig(String)} in order to find
     * an appropriate URL. {@link URL#openStream()} is then called to
     * obtain the stream.
     *
     * @param path The path representing the config location.
    ...
    public static final InputStream getConfigStream(final String path)
            throws GenaUtilException {
        final URL url = ConfigHelper.locateConfig(path);
        if (url == null) {
            String msg = "Unable to locate config file: " + path;
            throw new GenaUtilException(msg);
        }
        try {
            return url.openStream();
        } catch (IOException e) {
            throw new GenaUtilException("Unable to open config file: " + path,
                    e);
        }
    }
     ...
     * call to {@link #locateConfig(String)} in order to find an
     * appropriate URL. {@link URL#openStream()} is then called to
    ...
    public static InputStream getResourceAsStream(String resource)
        ...
        if (stream == null) {
            ConfigHelper.class.getResourceAsStream(resource);
        }
        if (stream == null) {
        ...
        if (stream == null) {
            throw new GenaUtilException(resource + " not found");
        }
        return stream;
    }
}